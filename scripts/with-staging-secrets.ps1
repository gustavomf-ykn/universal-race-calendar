param([ValidateSet('check','migrate','audit','auth','flow','corridasbr','recovery','batch','remote','api','calendar','results')][string]$Action='check',[Parameter(Mandatory=$true)][ValidatePattern('^[a-z]{20}$')][string]$ProjectRef)
