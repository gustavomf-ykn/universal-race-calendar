param([ValidateSet('check','migrate','audit','auth','flow','corridasbr','recovery','batch','remote','localflow','api','calendar','results','executors')][string]$Action='check',[Parameter(Mandatory=$true)][ValidatePattern('^[a-z]{20}$')][string]$ProjectRef)
...
try {
 ...
 switch($Action){
  ...
  'executors' { & python scripts/local-executors.py }
